...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void optionalEveryAttendeeIsConsideredNoChange() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
    // The addition of optional attendee C should not change the results.
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 2", TimeRange.fromStartDuration(TIME_0900AM, DURATION_30_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartDuration(TimeRange.START_OF_DAY, DURATION_24_HOUR),
  ...
  @Test
  public void optionalEveryAttendeeIsConsideredChange() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
    // Due to optional attendee C, only options 1 and 3 are returned.
  ...
  @Test
  public void optimizeOptionalJustEnoughRoom() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting.
    //
    // Events  : |--A--|     |----A----|
  ...
  @Test
  public void optimizeEveryAttendeeIsConsideredChange() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 4", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0800AM, false),
            Arrays.asList(PERSON_C, PERSON_E)),
  ...
  @Test
  public void optimizeNoMandatoryAttendees() {
    Collection<Event> events = Arrays.asList(
  ...
  @Test
  public void optimizeIncreaseBoundary() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    ...
    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected =
        Arrays.asList(TimeRange.fromStartEnd(TIME_0830AM, TIME_0930AM, true));
